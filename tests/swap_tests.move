...
module brownfi_amm::swap_tests {
    ...
    #[test]
    #[expected_failure(abort_code = swap::EPoolAlreadyExists)]
    fun test_create_pool_aborts_on_duplicate_pair() {
        ...
        next_tx(&mut scenario, ADDR1);
        {
            ...
            let lp = swap::create_pool(&mut factory, init_a, init_b, ctx); // aborts here
            transfer::public_transfer(coin::from_balance(lp, ctx), sender(ctx));
    ...
    #[test]
    #[expected_failure(abort_code = swap::EInvalidPair)]
    ...
    fun test_add_liquidity_aborts_on_min_lp_out() {
        ...
        next_tx(&mut scenario, ADDR2);
        {
            ...
            let (remaining_a, remaining_b, lp) = swap::add_liquidity(&mut pool, input_a, input_b, 201); // aborts here
    ...
    fun test_remove_liquidity_aborts_on_zero_input_lp() {
        ...
        next_tx(&mut scenario, ADDR1);
        {
            ...
            let lp = balance::zero();
            let (a_out, b_out) = swap::remove_liquidity(&mut pool, lp, 0, 0); // aborts here
    ...
    fun test_remove_liquidity_aborts_on_min_a_out() {
        ...
        next_tx(&mut scenario, ADDR1);
        {
            ...
            let (a_out, b_out) = swap::remove_liquidity(&mut pool, lp_in, 51, 50); // aborts here
    ...
    fun test_remove_liquidity_aborts_on_min_b_out() {
        ...
        next_tx(&mut scenario, ADDR1);
        {
            ...
            let lp_in = coin::into_balance(coin::split(&mut lp_coin, 50, ctx));
            let (a_out, b_out) = swap::remove_liquidity(&mut pool, lp_in, 50, 51); // aborts here
